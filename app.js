...
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
  ...
  fetch("changelog.html", { cache: "no-cache" }).then(async (c) => {
    let r = await c.text(),
      p = new DOMParser().parseFromString(r, "text/html");
    document.querySelector("#patchNotes").innerHTML +=
...
window.onload = async () => {
  ...
  function drawMinimapAndDebug(spacing, alcoveSize, GRAPHDATA) {
    ...
    if (global.showDebug) {
      ...
      drawText(
        global.metrics.latency + " ms - " + global.serverName.toLowerCase(),
        x + len,
        y - 50,
        10,
        color.guiwhite,
  ...
  const gameDrawDead = () => {
    ...
    drawText(
      "🦆 The server was " + +(100 * gui.fps).toFixed(0) + "%" + " active",
      x - 170,
      y + 144,
      16,
      color.guiwhite
